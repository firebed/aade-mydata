<?php

namespace Firebed\AadeMyData\Models\Enums;

class WithheldTax
{
    /**
     *  Περιπτ. β’- Τόκοι - 15% 15%
     */
    public const TAX_1 = "1";
    

    /**
     *  Περιπτ. γ’ - Δικαιώματα - 20% 20%
     */
    public const TAX_2 = "2";
    

    /**
     *  Περιπτ. δ’ - Αμοιβές Συμβουλών Διοίκησης - 20% 20%
     */
    public const TAX_3 = "3";
    

    /**
     *  Περιπτ. δ’ - Τεχνικά Έργα - 3% 3%
     */
    public const TAX_4 = "4";
    

    /**
     *  Υγρά καύσιμα και προϊόντα καπνοβιομηχανίας 1% 1%
     */
    public const TAX_5 = "5";
    

    /**
     *  Λοιπά Αγαθά 4% 4%
     */
    public const TAX_6 = "6";
    

    /**
     *  Παροχή Υπηρεσιών 8% 8%
     */
    public const TAX_7 = "7";
    

    /**
     *  Προκαταβλητέος Φόρος Αρχιτεκτόνων και Μηχανικών επί Συμβατικών Αμοιβών, για Εκπόνηση Μελετών και Σχεδίων 4% 4%
     */
    public const TAX_8 = "8";
    

    /**
     *  Προκαταβλητέος Φόρος Αρχιτεκτόνων και Μηχανικών επί Συμβατικών Αμοιβών, που αφορούν οποιασδήποτε άλλης φύσης έργα 10% 10%
     */
    public const TAX_9 = "9";
    

    /**
     *  Προκαταβλητέος Φόρος στις Αμοιβές Δικηγόρων 15% 15%
     */
    public const TAX_10 = "10";
    

    /**
     *  Παρακράτηση Φόρου Μισθωτών Υπηρεσιών παρ. 1 αρ. 15 ν. 4172/2013 ποσό
     */
    public const TAX_11 = "11";
    

    /**
     *  Παρακράτηση Φόρου Μισθωτών Υπηρεσιών παρ. 2 αρ. 15 ν. 4172/2013 - Αξιωματικών Εμπορικού Ναυτικού 15%
     */
    public const TAX_12 = "12";
    

    /**
     *  Παρακράτηση Φόρου Μισθωτών Υπηρεσιών παρ. 2 αρ. 15 ν. 4172/2013 - Κατώτερο Πλήρωμα Εμπορικού Ναυτικού 10%
     */
    public const TAX_13 = "13";
    

    /**
     *  Παρακράτηση Ειδικής Εισφοράς Αλληλεγγύης ποσό
     */
    public const TAX_14 = "14";
    

    /**
     *  Παρακράτηση Φόρου Αποζημίωσης λόγω Διακοπής Σχέσης Εργασίας παρ. 3 αρ. 15 ν. 4172/2013 ποσό
     */
    public const TAX_15 = "15";
    

    /**
     *  Παρακρατήσεις συναλλαγών αλλοδαπής βάσει συμβάσεων αποφυγής διπλής φορολογίας (Σ.Α.Δ.Φ.) ποσό
     */
    public const TAX_16 = "16";
    

    /**
     *  Λοιπές Παρακρατήσεις Φόρου ποσό
     */
    public const TAX_17 = "17";
}